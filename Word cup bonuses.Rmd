...
world_cup_bonuses <- ggplot(bonuses2, aes(x = number, y = cumulative)) +
  ...
  # add labels to x and y axis, title 
...
sum_women <- bonuses %>% summarize(sum_women = sum(women)) %>% pull()
sum_men <- bonuses %>% summarise(sum_men = sum(men)) %>% pull()
sum_diff <- bonuses %>% summarise(sum_diff = sum(men) - sum(women)) %>% pull()

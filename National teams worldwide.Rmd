...
graph_data <- wws2 %>% left_join(mws4, by = c("avg_salary" = "avg_annual_pay_country", "country" = "country_clean", "Gender" = "Gender", "year" = "year")) %>% 
  select(avg_salary, country, year, Gender)
```

```{r}
ggplot(graph_data) +
  geom_col(aes(x = country, y = avg_salary, group = Gender))
